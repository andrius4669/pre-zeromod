...
//----------------------------------------------------------
    ...
    int _noselfdamage = 0;
	//float values
	float _damageboost = 1.0;
	float _hitpush = 1.0;
	float _selfhitpush = 1.0;
	//other type values
	char *_botname = NULL;
    
    vector<clientinfo *> _mapreceivers;
    clientinfo *_ghostclient = NULL;
//----------------------------------------------------------
...
//------------------------------------
        void reset()
        {
            ...
            maxhealth = 100;
			if(_maxhealth >= 0) maxhealth = _maxhealth;
			if((aitype != AI_NONE) && (_botmaxhealth >= 0)) maxhealth = _botmaxhealth;
			_hitclient = -1;
            rockets.reset();
            grenades.reset();
...
//		if(name[0] && !duplicatename(ci, name) && ci->state.aitype == AI_NONE && !ci->privilege) return name;
...
//		if(ci->privilege)
//		{
//			formatstring(cname[cidx])("\fs\f4%s\fr \fs\f5[%d]\fr", name, ci->clientnum);
//		}
//		else
//		{
//			formatstring(cname[cidx])(ci->state.aitype == AI_NONE ? "%s \fs\f5(%d)\fr" : "%s \fs\f5[%d]\fr", name, ci->clientnum);
...
//--------------------------------------------------------------------
    ...
    {
		if(_nopickup) return false;
        if((m_timed && gamemillis>=gamelimit) || !sents.inrange(i) || !sents[i].spawned) return false;
        clientinfo *ci = getinfo(sender);
		if((ci->state.aitype != AI_NONE) && _nobotpickup) return false;
...
//--------------------------------------------------------------------------------------
    void spawnstate(clientinfo *ci)
    {
		...
		if(_maxhealth >= 0) gs.health = _maxhealth;
        if((gs.aitype != AI_NONE) && (_botmaxhealth >= 0)) gs.health = _botmaxhealth;
		
        if(_maxarmour >= 0) gs.armour = _maxarmour;
        if((gs.aitype != AI_NONE) && (_botmaxarmour >= 0)) gs.armour = _botmaxarmour;
		
        if(_armourtype >= 0) gs.armourtype = _armourtype;
        if((gs.aitype != AI_NONE) && (_botarmourtype >= 0)) gs.armourtype = _botarmourtype;
		
        if(_ammoset) for(int i = 0; i < NUMGUNS; i++) if(_ammo[i] >= 0) gs.ammo[i] = _ammo[i];
        if((gs.aitype != AI_NONE) && _botammoset) for(int i = 0; i < NUMGUNS; i++) if(_botammo[i] >= 0) gs.ammo[i] = _botammo[i];

		if(_defammo >= 0) gs.gunselect = _defammo;
		if((gs.aitype != AI_NONE) && (_defbotammo >= 0)) gs.gunselect = _defbotammo;
...
//--------------------------------------------------------------------------------------------------------------------
    void changemap(const char *s, int mode)
    {
        stopdemo();
        pausegame(false);
        changegamespeed((_gamespeed >= 0) ? _gamespeed : 100);
        if(smode) smode->cleanup();
        aiman::clearai();

        gamemode = mode;
        gamemillis = 0;
        if(_gamelimit >= 0) gamelimit = _gamelimit * 1000;
		else gamelimit = (m_overtime ? 15 : 10)*60000;
        interm = 0;
        nextexceeded = 0;
...
//-----------------------------------------------------------------------------------------
    void rotatemap(bool next)
    {
		...
		{
			string s1;
			copystring(s1, smapname);
			changemap(s1, gamemode);
			return;
		}
...
//---------------------------------------------------
    void checkintermission()
    {
        if(gamemillis >= gamelimit && !interm)
        {
            ...
            changegamespeed(100);
            if(_intermission >= 0) interm = gamemillis + _intermission;
			else interm = gamemillis + 10000;
        }
    }
//--------------------------------------
...
//-------------------------------------------------------------------------------------------
    ...
    {
		if(_killsuicide) if(target != actor) target->state._hitclient = actor->clientnum;
		damage = int(damage * _damageboost);
        gamestate &ts = target->state;
		bool _shp;
		clientinfo *cx = NULL;	//some client other than target
		
		_shp = (_selfhitpush >= 1.9 && target==actor);	//if selfhitpush strategy should be used
		if(_shp)
		{
			loopv(clients) if(clients[i] != target && clients[i]->state.state == CS_ALIVE)
			{
				cx = clients[i];
				break;
			}
...
//			{
//				_shp = false;
//				sendservmsgf("selfhitpush: hitpush is zero");
//			}
			if(!cx)
			{
				_shp = false;
//				sendservmsgf("selfhitpush: no other clients");
			}
		}
		
//		if(_shp) sendservmsgf("selfhitpush: %i %i %i", hitpush.x, hitpush.y, hitpush.z);
		
        if(!(_nodamage || (_noselfdamage && (target==actor))))
		{
			if(!_shp) ts.dodamage(damage);
			if(target!=actor && !isteam(target->team, actor->team)) actor->state.damage += damage;
		}
		
		if(_shp)
		{
			sendf(-1, 1, "ri6", N_DAMAGE, target->clientnum, actor->clientnum, 0, ts.armour, ts.health);
			if(!(_nodamage || (_noselfdamage && (target==actor)))) ts.dodamage(damage);
		}
		else sendf(-1, 1, "ri6", N_DAMAGE, target->clientnum, actor->clientnum, damage, ts.armour, ts.health);

        if(target==actor)
		{
			if(_shp)
			{
				sendf(-1, 1, "ri6", N_DAMAGE, target->clientnum, cx->clientnum, damage, ts.armour, ts.health);
				
				sendf(ts.health<=0 ? -1 : target->ownernum, 1, "ri7", N_HITPUSH, target->clientnum, gun, damage,
					0, 0, int(_selfhitpush - 1.0));
			}
			target->setpushed();
		}
        else if(!hitpush.iszero())
        {
            ...
            sendf(ts.health<=0 ? -1 : target->ownernum, 1, "ri7", N_HITPUSH, target->clientnum, gun, damage,
				  int(v.x * _hitpush), int(v.y * _hitpush), int(v.z * _hitpush));
            target->setpushed();
        }
        if(ts.health<=0)
        {
...
//--------------------------------------------------------------------
    void suicide(clientinfo *ci)
    {
		...
		if(_killsuicide)
		{
			loopv(clients) if(ci->state._hitclient == clients[i]->clientnum) cx = clients[i];
			
			fragvalue = (cx ? 0 : (smode ? smode->fragvalue(ci, ci) : -1));
			if(cx)
			{
				int fv;
				fv = smode ? smode->fragvalue(ci, cx) : 1;
				cx->state.frags += fv;
				ti = m_teammode ? teaminfos.access(ci->team) : NULL;
				if(ti) ti->frags += fv;
			}
		}
		else fragvalue = smode ? smode->fragvalue(ci, ci) : -1;
        
        ci->state.frags += fragvalue;
        ci->state.deaths++;
        teaminfo *t = m_teammode ? teaminfos.access(ci->team) : NULL;
        if(t) t->frags += fragvalue;
        sendf(-1, 1, "ri5", N_DIED, ci->clientnum,
			  (cx ? cx : ci)->clientnum, (cx ? cx->state : gs).frags,
			  cx ? (ti ? ti->frags : 0) : (t ? t->frags : 0));
        ci->position.setsize(0);
...
//      sendf(ci->clientnum, 1, "ri5ss", N_SERVINFO, ci->clientnum, PROTOCOL_VERSION, ci->sessionid, serverpass[0] ? 1 : 0, serverdesc, serverauth);
		string str;
		formatstring(str)("\fs\f0[ %s ]\fr", serverdesc);
		sendf(ci->clientnum, 1, "ri5ss", N_SERVINFO, ci->clientnum, PROTOCOL_VERSION, ci->sessionid, serverpass[0] ? 1 : 0,
			  str, serverauth);
    }
//-------------------------------------------------------------------------------
...
//******************************************
    void clientdisconnect(int n)
    {
        ...
        if(ci == _ghostclient) _ghostclient = NULL;
...
//*******************************************
    int _sendmap(clientinfo *ci)
    {
        if(!mapdata || ci->getmap) return 0;

        if((ci->getmap = sendfile(ci->clientnum, 2, mapdata, "ri", N_SENDMAP))) ci->getmap->freeCallback = freegetmap;
...
//-------------------------------------------
    void receivefile(int sender, uchar *data, int len)
    {
        ...
        if(ci == _ghostclient)
        {
            loopv(_mapreceivers) _sendmap(_mapreceivers[i]);
            _mapreceivers.setsize(0);
        }
        else
        {
            if(!_ghostclient && (_mapreceivers.length() > 0))
            {
                loopv(_mapreceivers) _sendmap(_mapreceivers[i]);
                _mapreceivers.setsize(0);
            }
            sendservmsgf("[%s sent a map to server, \"/getmap\" to receive it]", colorname(ci));
            if(_ghostclient) sendf(_ghostclient->clientnum, 1, "ris", N_SERVCMD, "getmap");
        }
    }
...
//--------------------------------------------------------------------------------
    void connected(clientinfo *ci)
    {
        ...
        ci->connected = true;
        ci->needclipboard = totalmillis ? totalmillis : 1;
        if(mastermode>=MM_LOCKED || ((_maxplayers >= 0) && (numclients(-1, true, true) >= _maxplayers))) ci->state.state = CS_SPECTATOR;
        ci->state.lasttimeplayed = lastmillis;
...
//-------------------------------------------------------------------------------*
	int _argsep(char *str, int c, char *argv[])	//separate args (str = source string; c = expected argc; argv = ptrs array; ret = argc)
	{
		char *s;
		int argc;

		for(int i = 1; i < c; i++) argv[i] = 0;
	...
	int _strtogun(char *str)
	{
		int j = -1;

		if(str) if(*str != 0) if(strcmp(str, "all")) j = atoi(str);
		if(j == 0)
		{
			if(!strcmp(str, "0") || !strcmp(str, "fist")) j = GUN_FIST;	//really 0 :)
			else if(!strcmp(str, "sg") || !strcmp(str, "shotgun")) j = GUN_SG;
			else if(!strcmp(str, "cg") || !strcmp(str, "chaingun")) j = GUN_CG;
			else if(!strcmp(str, "rl") || !strcmp(str, "rockets")) j = GUN_RL;
			else if(!strcmp(str, "rifle")) j = GUN_RIFLE;
			else if(!strcmp(str, "gl") || !strcmp(str, "grenades")) j = GUN_GL;
			else if(!strcmp(str, "pistol")) j = GUN_PISTOL;
			else j = -2;
		}
		return j;
	}
	bool _privilege(clientinfo *ci)
	{
		return (_privl) ? ((ci->privilege > PRIV_MASTER) || ci->local) : (ci->privilege || ci->local);
	}
	bool _getbool(char *str, bool def = true)
	{
		int k;
		if(!str) return def;
		if(*str == 0) return def;
		k = atoi(str);
		if(k) k = 1;
		else if(!strcmp(str, "on") || !strcmp(str, "yes") || !strcmp(str, "true") || !strcmp(str, "enable")) k = 1;
		else if(!strcmp(str, "off") || !strcmp(str, "no") || !strcmp(str, "false") || strcmp(str, "disable") || !strcmp(str, "0")) k = 0;	
		else k = def;
		return k;
	}
	int _getint(char *str, int def = -1)
	{
		int k;
		if(!str) return def;
		if(*str == 0) return def;
		k = atoi(str);
		if(!k) if(strcmp(str, "0")) k = def;
		return k;
	}
//-------------------------------------------
	void _servcmd(clientinfo *ci, char *text)	//~~~~~~~~~~~~~~~~~~~~
	{
		char str[MAXTRANS];
		int argc;
		char *argv[4];
...
//		ci->privilege || ci->local
		...
		argc = _argsep(str, 2, argv);

//		sendservmsgf("[DBG] argv0=%s", argv[0]);
//		sendservmsgf("[DBG] argv1=%s", argv[1]);

		if(!strcmp(argv[0], "wall"))
		{
			if(!_privilege(ci)) return;
			if(argv[1]) sendservmsg(argv[1]);
		}
		else if(!strcmp(argv[0], "sendto"))
        {
            clientinfo *cx = NULL;
            
            if(!_privilege(ci)) return;
            int j = _getint(argv[1], -1);
            if(j < 0)
            {
                if(!_noinfo) sendservmsgf("%s forced getmap for everyone", colorname(ci));
                loopv(clients) _sendmap(clients[i]);
            }
            else
            {
                loopv(clients) if(clients[i]->clientnum == j) { cx = clients[i]; break; }
                if(cx)
                {
                    if(!_noinfo) sendservmsgf("%s forced getmap for %s", colorname(ci), colorname(cx));
                    _sendmap(cx);
                }
                else
                {
                    string msg;
                    formatstring(msg)("Unknown client number \"%s\"", argv[1]);
                    sendf(ci->clientnum, 1, "ris", N_SERVMSG, msg);
                }
            }
        }
		else if(!strcmp(argv[0], "ghost") || !strcmp(argv[0], "ghostclient"))
        {
            if(!_privilege(ci)) return;
            _ghostclient = ci;
            if(!_noinfo) sendservmsgf("%s ghosted", colorname(ci));
        }
		else if(!strcmp(argv[0], "defammo"))
		{
			int j;
			...
			{
				string msg;
				formatstring(msg)("Unknown gun \"%s\"", argv[1]);
				sendf(ci->clientnum, 1, "ris", N_SERVMSG, msg);
				return;
			}
			...
			if(!_noinfo) sendservmsgf("%s set default gun to %i", colorname(ci), j);
		}
		else if(!strcmp(argv[0], "defbotammo"))
		{
			int j;
			...
			if(!_noinfo) sendservmsgf("%s set bots default gun to %i", colorname(ci), j);
		}
		else if(!strcmp(argv[0], "shp") || !strcmp(argv[0], "selfhitpush"))
		{
			if(!_privilege(ci)) return;
			if(argv[1]) _selfhitpush = atof(argv[1]);
			else _selfhitpush = 1.0;
			if(!_noinfo) sendservmsgf("%s set SelfHitPush to %f", colorname(ci), _selfhitpush);
		}
		else if(!strcmp(argv[0], "keepmap"))
		{
			if(!_privilege(ci)) return;
			_keepmap = _getbool(argv[1]);
			if(!_noinfo) sendservmsgf("%s %sed KeepMap.", colorname(ci), _keepmap ? "enabl" : "disabl");
		}
		else if(!strcmp(argv[0], "botname"))
		{
			if(!_privilege(ci)) return;
			if(_botname) delete _botname;
			_botname = NULL;
			if(argv[1]) if(*argv[1])
			{
				_botname = new char[MAXNAMELEN+1];
				copystring(_botname, argv[1], MAXNAMELEN+1);
			}
			if(!_noinfo) sendservmsgf("%s set BotName to \"%s\".", colorname(ci), _botname);
		}
		else if(!strcmp(argv[0], "nodamage"))
		{
			if(!_privilege(ci)) return;
			_nodamage = _getbool(argv[1]);
			if(!_noinfo) sendservmsgf("%s %sed NoDamage.", colorname(ci), _nodamage ? "enabl" : "disabl");
		}
        else if(!strcmp(argv[0], "noselfdamage") || !strcmp(argv[0], "nsd"))
        {
            if(!_privilege(ci)) return;
            _noselfdamage = _getbool(argv[1]);
            if(!_noinfo) sendservmsgf("%s %sed NoSelfDamage.", colorname(ci), _noselfdamage ? "enabl" : "disabl");
        }
		else if(!strcmp(argv[0], "kill"))
		{
			...
			if(j >= 0)
			{
				loopv(clients) if(clients[i]->state.state == CS_ALIVE && clients[i]->clientnum == j) cx = clients[i];
				if(!cx)
				{
					string msg;
					formatstring(msg)("Alive client with id %s not found.", argv[1]);
					sendf(ci->clientnum, 1, "ris", N_SERVMSG, msg);
					return;
				}
			}
			else if(j == -2)
			{
				if(argv[1] && *argv[1])
				{
					loopv(clients) if(clients[i]->state.state == CS_ALIVE && !strcmp(argv[1], clients[i]->name)) { cx = clients[i]; break; }
					if(cx) j = cx->clientnum;
					else
					{
						string msg;
						formatstring(msg)("Alive client with name \"%s\" not found.", argv[1]);
						sendf(ci->clientnum, 1, "ris", N_SERVMSG, msg);
						return;
					}
				}
			...
			if(j >= 0)
			{
				t = m_teammode ? teaminfos.access(cx->team) : NULL;
				sendf(-1, 1, "ri5", N_DIED, j, j, cx->state.frags, t ? t->frags : 0);
				cx->position.setsize(0);
				cx->state.state = CS_DEAD;
				cx->state.lastdeath = gamemillis;
				cx->state.respawn();
				if(!_noinfo) sendservmsgf("%s forcefully killed %s.", colorname(ci), colorname(cx));
			}
			else	//killall
			{
				int k = 0;
				loopv(clients) if(clients[i]->state.state == CS_ALIVE)
				{
					k++;
					cx = clients[i];
					t = m_teammode ? teaminfos.access(cx->team) : NULL;
					sendf(-1, 1, "ri5", N_DIED, cx->clientnum, cx->clientnum, cx->state.flags, t ? t->frags : 0);
					cx->position.setsize(0);
					cx->state.state = CS_DEAD;
					cx->state.lastdeath = gamemillis;
					cx->state.respawn();
				}
				if(!_noinfo) sendservmsgf("%s forcefully killed %i clients.", colorname(ci), k);
			}
		}
		else if(!strcmp(argv[0], "maxplayers"))
		{
			if(!_privilege(ci)) return;
			_maxplayers = _getint(argv[1]);
			if(!_noinfo) sendservmsgf("%s set maximum active players count to %i.", colorname(ci), _maxplayers);
		}
		else if(!strcmp(argv[0], "gamespeed"))
		{
			if(!_privilege(ci)) return;
			_gamespeed = _getint(argv[1]);
			if(!_noinfo) sendservmsgf("%s set default gamespeed to %i.", colorname(ci), _gamespeed);
		}
		else if(!strcmp(argv[0], "intermission") || !strcmp(argv[0], "interm"))
		{
			if(!_privilege(ci)) return;
			_intermission = _getint(argv[1]);
			if(!_noinfo) sendservmsgf("%s set intermission interval to %i millisecconds.", colorname(ci), _intermission);
		}
		else if(!strcmp(argv[0], "gamelimit") || !strcmp(argv[0], "gl"))
		{
			if(!_privilege(ci)) return;
			_gamelimit = _getint(argv[1]);
			if(!_noinfo) sendservmsgf("%s set game length to %i secconds.", colorname(ci), _gamelimit);
		}
		else if(!strcmp(argv[0], "noinfo"))
		{
			if(!_privilege(ci)) return;
			_noinfo = _getbool(argv[1]);
			if(!_noinfo) sendservmsgf("%s disabled NoInfo.", colorname(ci));
		}
		else if(!strcmp(argv[0], "privl"))
		{
			if(!(ci->privilege > PRIV_MASTER) && !ci->local) return;
			_privl = _getbool(argv[1], false);
			if(!_noinfo) sendservmsgf("%s %sed privilege restrictions.", colorname(ci), _privl ? "enabl" : "disabl");
		}
		else if(!strcmp(argv[0], "armourtype") || !strcmp(argv[0], "at"))
		{
			int j;
			...
			if(argv[1])
			{
				...
				if(!j)
				{
					if(!strcmp(argv[1], "0") || !strcmp(argv[1], "none")) j = 0;
					else if(!strcmp(argv[1], "blue")) j = A_BLUE;
					else if(!strcmp(argv[1], "green")) j = A_GREEN;
					else if(!strcmp(argv[1], "yellow")) j = A_YELLOW;
					else j = -1;
				}
			}
			else j = -1;
			...
			if(!_noinfo)
			{
				if(j < 0) sendservmsgf("%s set ArmourType to defaults.", colorname(ci));
				else sendservmsgf("%s set ArmourType to %i.", colorname(ci), _armourtype);
			}
		}
		else if(!strcmp(argv[0], "botarmourtype") || !strcmp(argv[0], "bat"))
		{
			int j;
			...
			if(!_noinfo)
			{
				if(j < 0) sendservmsgf("%s set BotArmourType to defaults.", colorname(ci));
				else sendservmsgf("%s set BotArmourType to %i.", colorname(ci), _botarmourtype);
			}
		}
		else if(!strcmp(argv[0], "nosuicide"))
		{
			if(!_privilege(ci)) return;
			_nosuicide = _getbool(argv[1]);
			if(!_noinfo) sendservmsgf("%s %sed NoSuicide.", colorname(ci), _nosuicide ? "enabl" : "disabl");
		}
		else if(!strcmp(argv[0], "nobotpickup"))
		{
			if(!_privilege(ci)) return;
			_nobotpickup = _getbool(argv[1]);
			if(!_noinfo) sendservmsgf("%s %sed NoBotPickup.", colorname(ci), _nobotpickup ? "enabl" : "disabl");
		}
		else if(!strcmp(argv[0], "nopickup"))
		{
			if(!_privilege(ci)) return;
			_nopickup = _getbool(argv[1]);
			if(!_noinfo) sendservmsgf("%s %sed NoPickup.", colorname(ci), _nopickup ? "enabl" : "disabl");
		}
		else if(!strcmp(argv[0], "execute") || !strcmp(argv[0], "do") || !strcmp(argv[0], "exec"))
		{
			int i;
			if(!(ci->privilege > PRIV_MASTER) && !ci->local) return;
			
			if(!_noinfo) sendservmsgf("%s executes \"%s\".", colorname(ci), argv[1]);
			i = execute(argv[1]);
			if(!_noinfo) sendservmsgf("%s execution return code %i.", colorname(ci), i);
			else
			{
				string msg;
				formatstring(msg)("%s execution return code %i", colorname(ci), i);
				sendf(ci->clientnum, 1, "ris", N_SERVMSG, msg);
			}
		}
		else if(!strcmp(argv[0], "health") || !strcmp(argv[0], "h"))
		{
			int j;
			if(!_privilege(ci)) return;
//			if(argv[1])
//			{
//				j = atoi(argv[1]);
//				if(!j) if(strcmp(argv[1], "0")) j = (_maxhealth >= 0) ? _maxhealth : 100;
//			}
//			else j = (_maxhealth >= 0) ? _maxhealth : 100;
			j = _getint(argv[1], (_maxhealth >= 0) ? _maxhealth : 100);
			loopv(clients)
			{
				clients[i]->state.health = j;
				//notify using damage message
				sendf(-1, 1, "ri6", N_DAMAGE, clients[i]->clientnum, clients[i]->clientnum,
					0, clients[i]->state.armour, clients[i]->state.health);
			}
			if(!_noinfo) sendservmsgf("%s set all players health to %i", colorname(ci), j);
		}
		else if(!strcmp(argv[0], "armour") || !strcmp(argv[0], "a"))
		{
			int j;
			if(!_privilege(ci)) return;
//			if(argv[1])
//			{
//				j = atoi(argv[1]);
//				if(!j) if(strcmp(argv[1], "0")) j = (_maxarmour >= 0) ? _maxarmour : 0;
//			}
//			else j = (_maxarmour >= 0) ? _maxarmour : 0;
			j = _getint(argv[1], (_maxarmour >= 0) ? _maxarmour : 0);
			loopv(clients)
			{
				clients[i]->state.armour = j;
				sendf(-1, 1, "ri6", N_DAMAGE, clients[i]->clientnum, clients[i]->clientnum,
					0, clients[i]->state.armour, clients[i]->state.health);
			}
			if(!_noinfo) sendservmsgf("%s set all players armour to %i", colorname(ci), j);
		}
		else if(!strcmp(argv[0], "sk") || !strcmp(argv[0], "suicidekill"))
		{
			int j;
			if(!_privilege(ci)) return;
//			if(argc < 2) j = _killsuicide ? 0 : 1;
//			else
//			{
//				j = atoi(argv[1]);
//				if(j > 0) j = 1;
...
//				{
//					if(j < 0) j = 0;
//					else if(!strcmp(argv[1], "on") || !strcmp(argv[1], "yes") || !strcmp(argv[1], "true")) j = 1;
//					else j = 0;
//				}
//			}
			j = _getbool(argv[1]);
			...
			{
				_killsuicide = 0;
				loopv(clients) clients[i]->state._hitclient = -1;
			}
			if(!_noinfo) sendservmsgf("%s %sed SuicideKill.", colorname(ci), j ? "enabl" : "disabl");
		}
		else if(!strcmp(argv[0], "ammo"))
		{
			int j, k;

			if(!_privilege(ci)) return;
			strcpy(str, text);
			argc = _argsep(str, 4, argv);
			if(!_ammoset) {for(int i = 0; i < NUMGUNS; i++) _ammo[i] = -1; _ammoset = 1;}
			if(argc == 1)
			{
				_ammoset = 0;
				if(!_noinfo) sendservmsgf("%s set default ammo config to defaults.", colorname(ci));
			}
			else
			{
				k = -1;
				...
				if(argv[2]) if(*argv[2] != 0) k = atoi(argv[2]);
				...
				if(j < 0)
				{
					for(int i = 0; i < NUMGUNS; i++) _ammo[i] = k;
					if(!_noinfo) sendservmsgf("%s set all guns default ammo to %i", colorname(ci), k);
				}
				else
				{
					_ammo[j] = k;
					if(!_noinfo) sendservmsgf("%s set gun %i default to %i", colorname(ci), j, k);
				}
			}
		}
		else if(!strcmp(argv[0], "botammo"))
		{
			int j, k;
			...
			strcpy(str, text);
			argc = _argsep(str, 4, argv);
			if(!_botammoset) {for(int i = 0; i < NUMGUNS; i++) _botammo[i] = -1; _botammoset = 1;}
			if(argc == 1)
			{
				_botammoset = 0;
				if(!_noinfo) sendservmsgf("%s set default bots ammo config to defaults.", colorname(ci));
			}
			else
			{
				k = -1;
				...
				if(j < 0)
				{
					for(int i = 0; i < NUMGUNS; i++) _botammo[i] = k;
					if(!_noinfo) sendservmsgf("%s set all bots guns default ammo to %i", colorname(ci), k);
				}
				else
				{
					_botammo[j] = k;
					if(!_noinfo) sendservmsgf("%s set bots gun %s default ammo to %i", colorname(ci), argv[1], k);
				}
			}
		}
		else if(!strcmp(argv[0], "db") || !strcmp(argv[0], "damageboost"))
		{
			if(!_privilege(ci)) return;
			if(argv[1]) _damageboost = atof(argv[1]);
			else _damageboost = 1.0;
			if(!_noinfo) sendservmsgf("%s set DamageBoost to %f", colorname(ci), _damageboost);
		}
		else if(!strcmp(argv[0], "mh") || !strcmp(argv[0], "maxhealth"))
		{
			if(!_privilege(ci)) return;
//			if(argv[1])
//			{
//				_maxhealth = atoi(argv[1]);
//				if(!_maxhealth) if(strcmp(argv[1], "0")) _maxhealth = -1;
//			}
//			else _maxhealth = -1;
			_maxhealth = _getint(argv[1]);
			loopv(clients)
			{
				clients[i]->state.maxhealth = (_maxhealth >= 0) ? _maxhealth : 100;
			}
			if(!_noinfo) sendservmsgf("%s set MaxHealth to %i", colorname(ci), _maxhealth);
		}
		else if(!strcmp(argv[0], "ma") || !strcmp(argv[0], "maxarmour"))
		{
			if(!_privilege(ci)) return;
			if(argv[1]) _maxarmour = atoi(argv[1]);
			else _maxarmour = -1;
			if(!_noinfo) sendservmsgf("%s set Armour to %i", colorname(ci), _maxarmour);
		}
		else if(!strcmp(argv[0], "hp") || !strcmp(argv[0], "hitpush"))
		{
			if(!_privilege(ci)) return;
			if(argv[1]) _hitpush = atof(argv[1]);
			else _hitpush = 1.0;
			if(!_noinfo) sendservmsgf("%s set HitPush to %f", colorname(ci), _hitpush);
		}
		else if(!strcmp(argv[0], "users"))
		{
			string msg;
			loopv(clients)
			{
				formatstring(msg)("%i, %s, %s", clients[i]->clientnum, colorname(clients[i]),
					(clients[i]->state.aitype == AI_NONE ? "AI_NONE" : "Non AI_NONE"));
				sendf(ci->clientnum, 1, "ris", N_SERVMSG, msg);
			}
		}
		else if(!strcmp(argv[0], "help"))
		{
			if(!argv[1])
			{
				sendf(ci->clientnum, 1, "ris", N_SERVMSG, "Avaiable commands:");
				sendf(ci->clientnum, 1, "ris", N_SERVMSG, "users.");
				sendf(ci->clientnum, 1, "ris", N_SERVMSG, "For more details about command, type #help <command>.");
				sendf(ci->clientnum, 1, "ris", N_SERVMSG, "For commands avaiable for masters and admins, type #help admin.");
			}
			else
			{
				if(!strcmp(argv[1], "admin"))
				{
					sendf(ci->clientnum, 1, "ris", N_SERVMSG, "wall, damageboost(db), maxhealth(mh), maxarmour(ma), hitpush(hp), nodamage, selfhitpush(shp),");
					sendf(ci->clientnum, 1, "ris", N_SERVMSG, "suicidekill(sk), ammo, botammo, health(h), armour(a), nopickup, nobotpickup, botname, keepmap,");
					sendf(ci->clientnum, 1, "ris", N_SERVMSG, "nosuicide, noinfo, armourtype(at), botarmourtype(bat), gamelimit(gl), intermission(interm),");
					sendf(ci->clientnum, 1, "ris", N_SERVMSG, "maxplayers, defammo, defbotammo, noselfdamage(nsd).");
					sendf(ci->clientnum, 1, "ris", N_SERVMSG, "For more details about command, type #help <command>.");
				}
				else if(!strcmp(argv[1], "hp") || !strcmp(argv[1], "hitpush"))
				{
					sendf(ci->clientnum, 1, "ris", N_SERVMSG, "hitpush <float>");
					sendf(ci->clientnum, 1, "ris", N_SERVMSG, "Set HitPush multiplier to some value (default 1.0)");
					sendf(ci->clientnum, 1, "ris", N_SERVMSG, "Example: #hp 10");
				}
				else if(!strcmp(argv[1], "sk") || !strcmp(argv[1], "suicidekill"))
				{
					sendf(ci->clientnum, 1, "ris", N_SERVMSG, "suicidekill on/off - turns suicide->kill behavour on or off");
				}
				else
				{
					string msg;
					formatstring(msg)("No info about command \"%s\".", argv[1]);
					sendf(ci->clientnum, 1, "ris", N_SERVMSG, msg);
				}
			}
		}
//		N_SERVMSG
//		send(ci->clientnum, 1, "ris", N_SERVMSG, "bla bla bla");
//		else sendservmsgf("Unknown cmd \"%s\"", argv[0]);
		else
		{
			string msg;
			formatstring(msg)("Unknown command \"%s\".", argv[0]);
			sendf(ci->clientnum, 1, "ris", N_SERVMSG, msg);
			sendf(ci->clientnum, 1, "ris", N_SERVMSG, "For list of commands type #help");
		}
...
//-----------------------------------------------------------------------
            case N_SUICIDE:
            {
                if(!_nosuicide) if(cq) cq->addevent(new suicideevent);
                break;
            }
//--------------------------------------------------------------
...
//--------------------------------------------------------------------------
            case N_TEXT:
            {
				...
				if(text[0] == '#')
				{
//					if(cm && (!cm->local || demorecord || hasnonlocalclients())) while(curmsg<p.length()) curmsg++;
					cm->messages.drop();
					_servcmd(ci, text + 1);
				}
				else
...
//--------------------------------------------------------------------
					...
					(
						...
						(
							...
							(
								...
								(
									(_maxplayers >= 0) && (numclients(-1, true, true) >= _maxplayers)
								)
							)
						)
					)
            ...
            case N_GETMAP:
            {
                ...
                if(!mapdata)
                {
                    if(_ghostclient)
                    {
                        loopv(_mapreceivers) if(_mapreceivers[i] == ci) exists = 1;
                        if(!exists)
                        {
                            _mapreceivers.add(ci);
                            if(_mapreceivers.length() < 2) sendf(_ghostclient->clientnum, 1, "ris", N_SERVCMD, "sendmap");
                        }
                    }
                ...
                {
                    if(ci != _ghostclient) sendservmsgf("[%s is getting the map]", colorname(ci));
                    
                    if(_ghostclient && (_ghostclient != ci))
                    {
                        loopv(_mapreceivers) if(_mapreceivers[i] == ci) exists = 1;
                        if(!exists)
                        {
